...
plot <- 
  ...
  geom_point(aes(x = factor(year), 
    ...
    subtitle = "Sorted According to Run Bracket",
...
ggsave("win_stat_by_runs.png")
```

```{r printing plot, echo=FALSE}

knitr::include_graphics("win_stat_by_runs.png")

```

NOTE: Bracket indicates that less than 15 runs were scores in the first five overs, 30 indicates that between 15 and 30 runs were scored and 30 indicates that over 30 runs were scored